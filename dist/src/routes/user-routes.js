...
usersRoutes.get("/me", tokenMiddleware, async (context) => {
    try {
        ...
        if (error.message === GetMeError.BAD_REQUEST) {
            return context.json({ error: GetMeError.BAD_REQUEST }, 400);
        }
...
usersRoutes.get("", tokenMiddleware, async (context) => {
    try {
        ...
        return context.json({ data: result.users, pagination: result.pagination }, 200);
    }
    catch (error) {
        return context.json({ message: GetAllUsersError.INTERNAL_SERVER_ERROR }, 500);
    }
